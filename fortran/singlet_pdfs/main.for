...
c	meq = 3000.
  ...
  100   CALL SAMPLE(R,WGT,1,MANY,ITN,KN,40,DG)
...
c       write(*,*) x1
	...
	S=meq**2.
      ...
      if(x1 .GT. x2) then
c		fac1 = u1*ub2+c1*cb2
c		fac2 = u2*ub1+c2*cb1
		fac1 = d1*db2+s1*sb2
		fac2 = d2*db1+s2*sb1
c		fac1 = s1*sb2+c1*cb2
c		fac2 = s2*sb1+c2*cb1
	else
c		fac1 = u2*ub1+c2*cb1
c		fac2 = u1*ub2+c1*cb2
		fac1 = d2*db1+s2*sb1
		fac2 = d1*db2+s1*sb2
...
c		fac1 = s2*sb1+c2*cb1
	...
	wgt = wgt * (fac2)/(fac1+fac2) * (1-tau0)/x1
