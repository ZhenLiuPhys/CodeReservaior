...
CleanParamLists[] := Block[{
   ...
   eplsave = epl;
   ipl = MapAt[# //. MR$Restrictions /. InvParamRules //.PRIVATE`$TensIndRules/.{f_?(PRIVATE`NoTensQ[#] === True &)[ind__?(FreeQ[##, NTIndex]&)] :> PRIVATE`NoTensPutInd[f, {ind}]} /. NTIndex -> Index //. MR$Restrictions //. ParamRules &, #, 1]& /@ IParamList; 
   IParamList = Intersection[IParamList, ipl];
   IParamList = MapAt[# //. MR$Restrictions /. InvParamRules //.PRIVATE`$TensIndRules/.{f_?(PRIVATE`NoTensQ[#] === True &)[ind__?(FreeQ[##, NTIndex]&)] :> PRIVATE`NoTensPutInd[f, {ind}]} /. NTIndex -> Index //. MR$Restrictions //. ParamRules &, #, 2]& /@ IParamList; 
   IParamList = IParamList /. ParamRules;

   epl = MapAt[MapAt[# //. MR$Restrictions /. InvParamRules //.PRIVATE`$TensIndRules/.{f_?(PRIVATE`NoTensQ[#] === True &)[ind__?(FreeQ[##, NTIndex]&)] :> PRIVATE`NoTensPutInd[f, {ind}]} /. NTIndex -> Index //. MR$Restrictions //. ParamRules &, #, 1]&, #, 3]& /@ epl;
