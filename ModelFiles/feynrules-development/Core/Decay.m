...
TreatGLine[sp_,dirac__,sp2_]:=Block[{init,chain, factor,TDot,resu},
  ...
  chain= Cases[init,_?(MatchQ[#,(ProjM|ProjP|Ga)[ar__]]||MatchQ[#,ProjM|ProjP|Ga]&)];
...
CreateAmp[expr_]:=Block[{fields=expr[[1]]/.{fi_?(FieldQ[#]===True&),_?NumericQ}:>fi,Gamma=Expand[expr[[2]]],struc,wf,myidx,eext, masstest,dum, result,TheIndex},
  ...
  If[struc=!=0,
    If[struc==={Index[Spin, 2], Index[Spin, 3]},
      wf=wf/.{
        List[a_,fi_?(Spin32FieldQ[#]===True&),b_]:>List[a,ToSpinor[fi,Position[fields,fi][[1,1]],struc] Polbar[Index[Lorentz,Ext[2]]],b],
        List[a__,fi_?(Spin32FieldQ[#]===True&)]:>List[a,ToSpinor[fi,Position[fields,fi][[1,1]],struc] Polbar[Index[Lorentz,Ext[3]]]],
        List[fi_?(Spin32FieldQ[#]===True&),b__]:>List[ToSpinor[fi,Position[fields,fi][[1,1]],struc] Pol[Index[Lorentz,Ext[1]]],b]
      };
...
(*      wf=wf/.{fi_?(FermionQ[#]===True&):>ToSpinor[fi,Position[fields,fi][[1,1]],struc]}; *)
  ...
  If[CleanAmp[fields[[1,3]], fields[[1,1]],fields[[1,2]]], result=Drop[result,{3}]];RES1=result;
  If[CleanAmp[fields[[1,2]], fields[[1,3]],fields[[1,1]]], result=Drop[result,{2}]];RES2=result;
  If[CleanAmp[fields[[1,1]], fields[[1,3]],fields[[1,2]]], result=Drop[result,{1}]];RES3=result;
...
SolveGammaTrace[expr_]:=Block[{tr,list, len,length,G5,gmunu,mySolveGammaTrace,resu,nu1,nu2,nu3},
  ...
  If[len===0,Return[4]];
  If[length===2,resu=list/.{tr[G5,G5]->4, tr[Ga[mu1_],Ga[mu2_]]->4 ME[mu1,mu2]}; MakeDiracRule[expr,resu];Return[resu]];
  ...
  If[Head[list]===tr,
    list=Plus@@Table[(-1)^(ii+1)(gmunu[list[[1]],list[[ii+1]]]/.Ga[mu_]->mu/.gmunu->ME) SolveGammaTrace[GLine@@Drop[list,{1,ii+1,ii}]] ,{ii,Length[list]-2}]+
      I/6 Eps[(list[[1]]/.Ga[mu_]->mu),nu1,nu2,nu3] SolveGammaTrace[GLine@@Join[List@@Most[Rest[list]],{Ga[nu1],Ga[nu2],Ga[nu3]}]]
  ];
...
Conj[GLine[Spinor[ty1_,Ext[i1_]],ar___,Spinor[ty2_,Ext[i2_]]]]:=GLine[Spinor[Conj[ty2],Ext[i2]], Sequence@@Reverse[{ar}], Spinor[Conj[ty1],Ext[i1]]];
...
Squaring[{fields__},expr_]:=Block[{Amp,CAmp,ECAmp,M2,masses,gammalist,CArule={}},
  ...
  (* The epsilons are set to zero since they are contracted with momenta and we have only three different mementa *)
  M2 = M2//.Colorrules;  
  M2 = Expand[
    M2/.Eps[Index[Lorentz, eext[ii__]], Index[Lorentz, eext[jj__]], Index[Lorentz,al__],Index[Lorentz,be__]] *
     Eps[Index[Lorentz, Ext[ii__]], Index[Lorentz, Ext[jj__]],Index[Lorentz,alp__],Index[Lorentz,bep__]]:>
    2 ME[Index[Lorentz,al],Index[Lorentz,bep]] ME[Index[Lorentz,alp],Index[Lorentz,be]]-
    2 ME[Index[Lorentz,al],Index[Lorentz,alp]] ME[Index[Lorentz,be],Index[Lorentz,bep]]
  ];
  M2=M2/.GLine[argus___]:>SolveGammaTrace[GLine[argus]]/.Eps[__]->0; 
  M2=PolRules[M2,masses];
