...
IntxnLog[m_,m_,p2_,n_]:=
  ...
  If[m==0,0,1]If[p2==0,0,1](If[m^2==p2,0,1]IntxnLog2[m,m,p2,n]+If[m^2==p2,1,0]IntxnLog2[m,m,m^2,n])/;(FreeQ[m,Mass]&&FreeQ[p2,FourMomentum]&&(Not[FreeQ[p2,FR$MU]]||p2===0));
...
IntxnLog[m1_,m2_,p2_,n_]:=
  ...
  If[m1==0,1,0]If[m2==0,0,1]If[p2==0,0,1](If[m2^2==p2,0,1]IntxnLog2[0,m2,p2,n]+If[m2^2==p2,1,0]IntxnLog2[0,m2,m2^2,n])+
  If[m1==0,0,1]If[m2==0,1,0]If[p2==0,1,0]IntxnLog2[m1,0,0,n]+
  If[m1==0,0,1]If[m2==0,1,0]If[p2==0,0,1](If[m1^2==p2,0,1]IntxnLog2[m1,0,p2,n]+If[m1^2==p2,1,0]IntxnLog2[m1,0,m1^2,n])+
  If[m1==0,0,1]If[m2==0,0,1]If[p2==0,1,0](If[m1==m2,1,0]IntxnLog2[m1,m1,0,n]+If[m1==m2,0,1]IntxnLog2[m1,m2,0,n])+
  If[m1==0,0,1]If[m2==0,0,1]If[p2==0,0,1]If[m1==m2,1,0](If[m1^2==p2,0,1]IntxnLog2[m1,m1,p2,n]+If[m1^2==p2,1,0]IntxnLog2[m1,m1,m1^2,n])+
  If[m1==0,0,1]If[m2==0,0,1]If[p2==0,0,1]If[m1==m2,0,1](If[p2==m1^2,0,1]If[p2==m2^2,0,1]IntxnLog2[m1,m2,p2,n]+If[p2==m1^2,0,1]If[p2==m2^2,1,0]IntxnLog2[m1,m2,m2^2,n]+
    If[p2==m1^2,1,0]If[p2==m2^2,0,1]IntxnLog2[m1,m2,m1^2,n])/;(FreeQ[m1,Mass]&&FreeQ[m2,Mass]&&FreeQ[p2,FourMomentum]&&(Not[FreeQ[p2,FR$MU]]||p2===0));

IntxnLog2[0,0,0,n_Integer]:=-2/FR$Eps*FR$IR/(n+1);

IntxnLog2[0,0,p2_,0]:=(-2 + If[p2>0,0,1]*Re[Log[-p2]] + If[p2>0,1,0]Re[(-I*Pi + Log[p2])])/;FreeQ[p2,FourMomentum];
IntxnLog2[0,0,p2_,1]:=(-2 +  (If[p2>0,0,1]*Re[Log[-p2]] + If[p2>0,1,0]Re[(-I*Pi + Log[p2])]))/2/;FreeQ[p2,FourMomentum];
IntxnLog2[0,0,p2_,2]:=(-13 + 6*(If[p2>0,0,1]*Re[Log[-p2]] + If[p2>0,1,0]Re[(-I*Pi + Log[p2])]))/18/;FreeQ[p2,FourMomentum];

IntxnLog2[0,m2_,0,0]:=(-1 + 2*Re[Log[m2]])/;FreeQ[m2,Mass];
IntxnLog2[0,m2_,0,1]:=(-3/4 + Re[Log[m2]])/;FreeQ[m2,Mass];
IntxnLog2[0,m2_,0,2]:=(-11/18 + (2*Re[Log[m2]])/3)/;FreeQ[m2,Mass];

IntxnLog2[m1_,0,0,0]:=(-1 + 2*Re[Log[m1]])/;FreeQ[m1,Mass];
IntxnLog2[m1_,0,0,1]:=(-1/4 + Re[Log[m1]])/;FreeQ[m1,Mass];
IntxnLog2[m1_,0,0,2]:=((-1 + 6*Re[Log[m1]])/9)/;FreeQ[m1,Mass];

IntxnLog2[m_,m_,0,0]:=Re[Log[m^2]]/;FreeQ[m,Mass];
IntxnLog2[m_,m_,0,1]:=Re[Log[m]]/;FreeQ[m,Mass];
IntxnLog2[m_,m_,0,2]:=(2*Re[Log[m]])/3/;FreeQ[m,Mass];

IntxnLog2[m_/FR$MU,0,m_^2/FR$MU^2,0]:=2*(-1 + Re[Log[m/FR$MU]])/;FreeQ[m,Mass];
IntxnLog2[m_/FR$MU,0,m_^2/FR$MU^2,1]:=(-1/2 + Re[Log[m/FR$MU]])/;FreeQ[m,Mass];
IntxnLog2[m_/FR$MU,0,m_^2/FR$MU^2,2]:=(2*(-1 + 3*Re[Log[m/FR$MU]]))/9/;FreeQ[m,Mass];

IntxnLog2[0,m_/FR$MU,m_^2/FR$MU^2,0]:=2*(-1 + Re[Log[m/FR$MU]])/;FreeQ[m,Mass];
IntxnLog2[0,m_/FR$MU,m_^2/FR$MU^2,1]:=(-3/2 + Re[Log[m/FR$MU]])/;FreeQ[m,Mass];
IntxnLog2[0,m_/FR$MU,m_^2/FR$MU^2,2]:=(-11 + 6*Re[Log[m/FR$MU]])/9/;FreeQ[m,Mass];

IntxnLog2[m1_,m2_,0,0]:=Re[(-m1^2 + m2^2 + 2*m1^2*Log[m1] - 2*m2^2*Log[m2])/(m1^2 - m2^2)]/;Not[m1===m2]&&FreeQ[m1,Mass]&&FreeQ[m2,Mass];
IntxnLog2[m1_,m2_,0,1]:=Re[(-m1^4 + 4*m1^2*m2^2 - 3*m2^4 + 4*(m1^4 - 2*m1^2*m2^2)*Log[m1] + 4*m2^4*Log[m2])/(4*(m1^2 - m2^2)^2)]/;Not[m1===m2]&&FreeQ[m1,Mass]&&FreeQ[m2,Mass];
IntxnLog2[m1_,m2_,0,2]:=Re[(-2*m1^6 + 9*m1^4*m2^2 - 18*m1^2*m2^4 + 11*m2^6 + 12*(m1^6 - 3*m1^4*m2^2 + 3*m1^2*m2^4)*Log[m1] - 12*m2^6*Log[m2])/(18*(m1^2 - m2^2)^3)]/;Not[m1===m2]&&FreeQ[m1,Mass]&&FreeQ[m2,Mass];

IntxnLog2[m1_,0,p2_,0]:=Re[(-2*p2 + 2*m1^2*Log[m1] + (-m1^2 + p2)*(If[m1^2<p2&&p2\[Element]Reals,1,0]*(Log[-m1^2 + p2]+I Pi)+If[m1^2<p2&&p2\[Element]Reals,0,1]*Log[m1^2 - p2]))/p2]/;FreeQ[p2,FourMomentum];
IntxnLog2[m1_,0,p2_,1]:=Re[1/2/p2(m1^2(2Log[m1]-1)-(IntxnLog2[m1,0,p2,0]/.Re->Identity)(m1^2-p2))]/;FreeQ[m1,Mass]&&FreeQ[p2,FourMomentum];
IntxnLog2[m1_,0,p2_,2]:=Re[((m1^2-p2/3)/2-2(m1^2-p2)(IntxnLog2[m1,0,p2,1]/.Re->Identity)-m1^2(1-2Log[m1]))/3/p2]/;FreeQ[m1,Mass]&&FreeQ[p2,FourMomentum];

IntxnLog2[0,m2_,p2_,0]:=Re[(-2*p2 + 2*m2^2*Log[m2] + (-m2^2 + p2)*(If[m2^2<p2&&p2\[Element]Reals,1,0]*(Log[-m2^2 + p2]+I Pi)+If[m2^2<p2&&p2\[Element]Reals,0,1]*Log[m2^2 - p2]))/p2]/;FreeQ[p2,FourMomentum];
IntxnLog2[0,m2_,p2_,1]:=Re[1/2/p2(-m2^2(2Log[m2]-1)-(IntxnLog2[0,m2,p2,0]/.Re->Identity)(-m2^2-p2))]/;FreeQ[m2,Mass]&&FreeQ[p2,FourMomentum];
IntxnLog2[0,m2_,p2_,2]:=Re[((m2^2-p2/3)/2-m2^2 (IntxnLog2[0,m2,p2,0]/.Re->Identity)-2(-m2^2-p2)(IntxnLog2[0,m2,p2,1]/.Re->Identity))/3/p2]/;FreeQ[m2,Mass]&&FreeQ[p2,FourMomentum];

IntxnLog2[m_/FR$MU,m_/FR$MU,m_^2/FR$MU^2,0]:=-2 + Pi/Sqrt[3] + 2*Re[Log[m/FR$MU]]/;FreeQ[m,Mass];
IntxnLog2[m_/FR$MU,m_/FR$MU,m_^2/FR$MU^2,1]:=-1 + Pi/(2*Sqrt[3]) + Re[Log[m/FR$MU]]/;FreeQ[m,Mass];
IntxnLog2[m_/FR$MU,m_/FR$MU,m_^2/FR$MU^2,2]:=-1/18 + (2*Re[Log[m/FR$MU]])/3/;FreeQ[m,Mass];

IntxnLog2[m_,m_,p2_,0]:=Re[-2 + Log[m^2] - (Sqrt[p2*(-4*m^2 + p2)]*(If[m\[Element]Reals&&p2\[Element]Reals&&m>0&&p2>4m^2,0,1]*Log[(2*m^2 - p2 + Sqrt[p2*(-4*m^2 + p2)])/(2*m^2)]+ 
  If[m\[Element]Reals&&p2\[Element]Reals&&m>0&&p2>4m^2,1,0](Log[-(2*m^2 - p2 + Sqrt[p2*(-4*m^2 + p2)])/(2*m^2)]+I Pi)))/p2]/;FreeQ[p2,FourMomentum];

IntxnLog2[m1_/FR$MU,m2_/FR$MU,m1_^2/FR$MU^2,0]:= Re[-2 + (-1 + m2^2/m1^2)*Log[m2/m1] + Log[(m1*m2)/FR$MU^2] - (Sqrt[-4*m1^2*m2^2 + m2^4]*
...
IntxnLog2[m1_,m2_,p2_,0]:=Re[-2 - ((m1^2 - m2^2)*Log[m2/m1])/p2 + Log[(m1*m2)/FR$MU^2] + 
...
IntxnLog2[m1_,m2_,p2_,1]:=Re[1/2/p2(-m2^2(2Log[m2]-1)+m1^2(2Log[m1]-1)-(IntxnLog2[m1,m2,p2,0]/.Re->Identity)(-m2^2+m1^2-p2))]/;FreeQ[m1,Mass]&&FreeQ[m2,Mass]&&
FreeQ[p2,FourMomentum];

IntxnLog2[m1_,m2_,p2_,2]:=Re[((m1^2+m2^2-p2/3)/2-m2^2 (IntxnLog2[m1,m2,p2,0]/.Re->Identity)-2(-m2^2+m1^2-p2)(IntxnLog2[m1,m2,p2,1]/.Re->Identity)-m1^2(1-2Log[m1]))/3/p2]/;
...
Dp2IntxnLog[m_,m_,p2_,n_]:=
  ...
  If[m==0,0,1]If[p2==0,0,1](If[m^2==p2,0,1]Dp2IntxnLog2[m,m,p2,n]+If[m^2==p2,1,0]Dp2IntxnLog2[m,m,m^2,n])/;FreeQ[m,Mass]&&FreeQ[p2,FourMomentum];
...
Dp2IntxnLog[m1_,m2_,p2_,n_]:=
  ...
  If[m1==0,1,0]If[m2==0,0,1]If[p2==0,0,1](If[m2^2==p2,0,1]Dp2IntxnLog2[0,m2,p2,n]+If[m2^2==p2,1,0]Dp2IntxnLog2[0,m2,m2^2,n])+
  If[m1==0,0,1]If[m2==0,1,0]If[p2==0,1,0]Dp2IntxnLog2[m1,0,0,n]+
  If[m1==0,0,1]If[m2==0,1,0]If[p2==0,0,1](If[m1^2==p2,0,1]Dp2IntxnLog2[m1,0,p2,n]+If[m1^2==p2,1,0]Dp2IntxnLog2[m1,0,m1^2,n])+
  If[m1==0,0,1]If[m2==0,0,1]If[p2==0,1,0](If[m1==m2,1,0]Dp2IntxnLog2[m1,m1,0,n]+If[m1==m2,0,1]Dp2IntxnLog2[m1,m2,0,n])+
  If[m1==0,0,1]If[m2==0,0,1]If[p2==0,0,1]If[m1==m2,1,0](If[m1^2==p2,0,1]Dp2IntxnLog2[m1,m1,p2,n]+If[m1^2==p2,1,0]Dp2IntxnLog2[m1,m1,m1^2,n])+
  If[m1==0,0,1]If[m2==0,0,1]If[p2==0,0,1]If[m1==m2,0,1](If[p2==m1^2,0,1]If[p2==m2^2,0,1]Dp2IntxnLog2[m1,m2,p2,n]+If[p2==m1^2,0,1]If[p2==m2^2,1,0]Dp2IntxnLog2[m1,m2,m2^2,n]+
    If[p2==m1^2,1,0]If[p2==m2^2,0,1]Dp2IntxnLog2[m1,m2,m1^2,n])/;FreeQ[m1,Mass]&&FreeQ[m2,Mass]&&FreeQ[p2,FourMomentum];

Dp2IntxnLog2[0,0,0,n_]:=0;

Dp2IntxnLog2[0,0,p2_,0]:=Re[1/p2]/;FreeQ[p2,FourMomentum];
Dp2IntxnLog2[0,0,p2_,1]:=Re[1/2/p2]/;FreeQ[p2,FourMomentum];
Dp2IntxnLog2[0,0,p2_,2]:=Re[1/3/p2]/;FreeQ[p2,FourMomentum];

Dp2IntxnLog2[m1_/FR$MU,0,m1_^2/FR$MU^2,0]:=-1(1/FR$Eps*FR$IR/Re[m1^2]+Re[(-1+Log[m1/FR$MU])/m1^2])FR$MU^2/;FreeQ[m1,FourMomentum];(*Infared divergent*)
Dp2IntxnLog2[m1_/FR$MU,0,m1_^2/FR$MU^2,1]:=-1/2/Re[m1^2]*FR$MU^2/;FreeQ[m1,FourMomentum];
Dp2IntxnLog2[m1_/FR$MU,0,m1_^2/FR$MU^2,2]:=-1/Re[m1^2]/6*FR$MU^2/;FreeQ[m1,FourMomentum];

Dp2IntxnLog2[0,m2_/FR$MU,m2_^2/FR$MU^2,0]:=-(1/FR$Eps*FR$IR/Re[m2^2]+Re[(-1+Log[m2/FR$MU])/m2^2])*FR$MU^2/;FreeQ[m2,FourMomentum];
Dp2IntxnLog2[0,m2_/FR$MU,m2_^2/FR$MU^2,1]:=-(1/FR$Eps*FR$IR/Re[m2^2]+Re[(-3/2+Log[m2/FR$MU])/m2^2])*FR$MU^2/;FreeQ[m2,FourMomentum];
Dp2IntxnLog2[0,m2_/FR$MU,m2_^2/FR$MU^2,2]:=-(1/FR$Eps*FR$IR/Re[m2^2]+Re[(-11/6+Log[m2/FR$MU])/m2^2])*FR$MU^2/;FreeQ[m2,FourMomentum];

Dp2IntxnLog2[m1_,0,p2_,0]:=Re[(p2 - 2*m1^2*Log[m1] + m1^2*(If[m1^2<p2&&p2\[Element]Reals,1,0]*(Log[-m1^2 + p2]+I Pi)+If[m1^2<p2&&p2\[Element]Reals,0,1]*Log[m1^2 - p2]))/p2^2]/;FreeQ[p2,FourMomentum];
Dp2IntxnLog2[m1_,0,p2_,1]:=Re[-(IntxnLog[m1,0,p2,1]/.Re->Identity)/p2+(IntxnLog[m1,0,p2,0]-(m1^2-p2)Dp2IntxnLog2[m1,0,p2,0]/.Re->Identity)/2/p2]/;FreeQ[m1,Mass]&&
FreeQ[p2,FourMomentum];
Dp2IntxnLog2[m1_,0,p2_,2]:=Re[-(IntxnLog[m1,0,p2,2]/Re->Identity)/p2+(-1/6+2IntxnLog[m1,0,p2,1]-2(m1^2-p2)Dp2IntxnLog2[m1,0,p2,1]/.Re->Identity)/3/p2]/;
FreeQ[m1,Mass]&&FreeQ[p2,FourMomentum];

Dp2IntxnLog2[0,m2_,p2_,0]:=Dp2IntxnLog2[m2,0,p2,0]/;FreeQ[p2,FourMomentum];
Dp2IntxnLog2[0,m2_,p2_,1]:=Re[- IntxnLog[0,m2,p2,1]/p2+(IntxnLog[0,m2,p2,0]-(-m2^2-p2)Dp2IntxnLog2[0,m2,p2,0])/2/p2/.Re->Identity]/;FreeQ[m2,Mass]&&
FreeQ[p2,FourMomentum];
Dp2IntxnLog2[0,m2_,p2_,2]:=Re[-IntxnLog[0,m2,p2,2]/p2+(-1/6-m2^2 Dp2IntxnLog2[0,m2,p2,0]+2IntxnLog[0,m2,p2,1]-2(-m2^2-p2)Dp2IntxnLog2[0,m2,p2,1])/3/p2/.Re->Identity]/;
FreeQ[m2,Mass]&&FreeQ[p2,FourMomentum];

Dp2IntxnLog2[m_/FR$MU,m_/FR$MU,0,n_Integer]:=-1/m^2*FR$MU^2/(n+2)/(n+3)/;FreeQ[m,Mass];

Dp2IntxnLog2[m_/FR$MU,m_/FR$MU,m_^2/FR$MU^2,0]:=(9 - 2*Sqrt[3]*Pi)/(9*Re[m^2])*FR$MU^2/;FreeQ[m,Mass];
Dp2IntxnLog2[m_/FR$MU,m_/FR$MU,m_^2/FR$MU^2,1]:=(9 - 2*Sqrt[3]*Pi)/(18*Re[m^2])*FR$MU^2/;FreeQ[m,Mass];
Dp2IntxnLog2[m_/FR$MU,m_/FR$MU,m_^2/FR$MU^2,2]:=(-6 + Sqrt[3]*Pi)/(9*Re[m^2])*FR$MU^2/;FreeQ[m,Mass];

Dp2IntxnLog2[m1_,m2_,0,0]:=Re[(-m1^4 + m2^4 + 4*m1^2*m2^2*Log[m1/m2])/(2*(m1^2 - m2^2)^3)]/;Not[m1===m2]&&FreeQ[m1,Mass]&&FreeQ[m2,Mass];
Dp2IntxnLog2[m1_,m2_,0,1]:=Re[-(m1^6 - 6*m1^4*m2^2 + 3*m1^2*m2^4 + 2*m2^6 - 12*m1^2*m2^4*Log[m2/m1])/(6*(m1^2 - m2^2)^4)]/;Not[m1===m2]&&FreeQ[m1,Mass]&&FreeQ[m2,Mass];
Dp2IntxnLog2[m1_,m2_,0,2]:=Re[(-m1^8 + 6*m1^6*m2^2 - 18*m1^4*m2^4 + 10*m1^2*m2^6 + 3*m2^8 + 24*m1^2*m2^6*Log[m1/m2])/(12*(m1^2 - m2^2)^5)]/;Not[m1===m2]&&FreeQ[m1,Mass]&&FreeQ[m2,Mass];

Dp2IntxnLog2[m_,m_,p2_,0]:=Re[(Sqrt[p2*(-4*m^2 + p2)] - 2*m^2*(If[m\[Element]Reals&&p2\[Element]Reals&&m>0&&p2>4m^2,0,1]*Log[(2*m^2 - p2 + Sqrt[p2*(-4*m^2 + p2)])/(2*m^2)]+ 
...
Dp2IntxnLog2[m1_/FR$MU,m2_/FR$MU,m1_^2/FR$MU^2,0]:= FR$MU^2*Re[((m1^2*Sqrt[-4*m1^2*m2^2 + m2^4] + (m1^2 - m2^2)*Sqrt[-4*m1^2*m2^2 + m2^4]*Log[m2/m1] + 
...
Dp2IntxnLog2[m1_,m2_,p2_,0]:=Re[((p2*Sqrt[m1^4 + (-m2^2 + p2)^2 - 2*m1^2*(m2^2 + p2)] + (m1^2 - m2^2)*Sqrt[m1^4 + (-m2^2 + p2)^2 - 2*m1^2*(m2^2 + p2)]*Log[m2/m1] + 
...
Dp2IntxnLog2[m1_,m2_,p2_,1]:=Re[(- IntxnLog[m1,m2,p2,1]/p2+(IntxnLog[m1,m2,p2,0]-(m1^2-m2^2-p2)Dp2IntxnLog2[m1,m2,p2,0])/2/p2)/.Re->Identity]/;FreeQ[m1,Mass]&&FreeQ[m2,Mass]&&
FreeQ[p2,FourMomentum];

Dp2IntxnLog2[m1_,m2_,p2_,2]:=Re[-IntxnLog[m1,m2,p2,2]/p2+(-1/6-m2^2 Dp2IntxnLog2[m1,m2,p2,0]+2IntxnLog[m1,m2,p2,1]-2(m1^2-m2^2-p2)Dp2IntxnLog2[m1,m2,p2,1])/3/p2/.Re->Identity]/;
FreeQ[m1,Mass]&&FreeQ[m2,Mass]&&FreeQ[p2,FourMomentum];
...
(*Print[InputForm[Union[Cases[fullUV[[kuv,2]],_IntxnLog,\[Infinity]]]]];*)
     ...
     eqlist=Append[eqlist,(Normal[Series[Refine[deq/.SP[FourMomentum[2],FourMomentum[2]]->TheMass[fullUV[[kuv,1,2,1]]]^2,
              Assumptions->Join[{FR$MU>0,FR$IR>0,FR$Eps>0},assumlist]],{FR$Eps,0,0}]])==0];,
     (*Same particle*)
     deq = Coefficient[fullUV[[kuv,2]],ME[Index[Lorentz,Ext[1]],Index[Lorentz,Ext[2]]]];Print[InputForm[deq]];
...
(*Print["before join"];Print[InputForm[If[Length[extra]>0,(#->0&)/@extra,{}]]];*)
  res=Join[res,Solve[eqlist/.{If->Cond,Re->ReInt},varlist][[1]],
  If[Length[extra]>0,(#->0&)/@extra,{}]];(*set to zero no usefull delta(Z)*)Print["solve done"];
];(*end for*)
...
R2Tadpoles[num_,del_,next_] := Block[{tmp, coef},
   ...
   Normal[Series[(tmp(1-FR$Eps/2)/.M$dim->4+FR$Eps),{FR$Eps,0,If[next<=2,1,0]}]]+ If[next<=2,1,0]*UVLog[del/FR$MU^2]/2*tmp/.{M$dim->4}))] /. {DTr -> MatrixTrace, ME -> MetricTensor, 
   FV -> FourVector}
];
...
GetR2[amp_,next_]:=Block[{temp,loopmom,nden,den,num,kk,extmom,intmom,lmcoef,DisFC,DisMT,DisDS,DisNC,nlmom,x,y,z,rl,lr,delta},
  loopmom=amp[[2,1]];
  ...
  Switch[nden,
    ...
    num=R2Tadpoles[num,den[[1,2]]^2,next];
    ,
    2,
...
(*Print["before mon integration"];*)
    num=R2BubblesQ2[num[[1]],loopmom,delta,next]+R2BubblesF[num[[2]],delta,next];
(*Print[InputForm[delta]];
Print["before x integration"];
Print[num];*)
    (*num = XIntegrate[num,delta,x];*)
    num=If[next===2,XIntegrate[num,delta,x],Integrate[num,{x,0,1}]];(*Print["end 2 propa"];*)
    ,
    3,
...
temp=SessionTime[];
  ...
  rlf={TensDot[gm:_Ga|_SlashedP,ProjP][Index[Spin,Ext[2]],Index[Spin,Ext[1]]]->TensDot[gm,ProjM][Index[Spin,Ext[1]],Index[Spin,Ext[2]]],
  ...
  For[kkpl=1,kkpl<=Length[genver]-0,kkpl++,Print[kkpl];
    ...
    If[Length[tmp]>0,
      ...
      If[Length[genver[[kkpl]]]===2,Print["before CTSol"];
        ...
        CTSol=Join[CTSol,SolveDelta[tmpCT,({#1,#2,Simplify[#3-Coefficient[#3,inveps,1]*inveps/.inveps->1/FR$Eps]}&)@@@(tmp/.FR$Eps->1/inveps),genver[[kkpl]]]];
      ...
      If[Length[genver[[kkpl]]]===1,
        ...
        tmp[[All,3]]=-I*(Simplify[Coefficient[#/.FR$Eps->1/inveps,inveps,1]/FR$Eps,TimeConstraint->10]&)/@tmp[[All,3]];];
...
vertlist=MergeVertList[vertlist,(vertCTlist//.CTSol)]/.{FourMomentum[xx_]->xx};
Print["merging done"];
