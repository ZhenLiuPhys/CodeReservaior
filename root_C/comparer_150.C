...
void comparer_150(){
  ...
  TFile *fs = new TFile("higgsinos_400_150.root");
  TFile *fs1 = new TFile("higgsinos_500_150.root");
  TFile *fs2 = new TFile("higgsinos_300_150.root");
  ...
  for (int j = 0; j < shists.size(); j++){
    bhists[j]->SetTitle("13 TeV, ttbar (red), mstop = 500, 400, 300 GeV, m_{#tilde{#tau}}= 150 GeV");
    bhists[j]->Draw();
    shists[j]->Draw("same");
    shists1[j]->Draw("same");
    shists2[j]->Draw("same");
  if (j == 0){
      c3->SaveAs("higgsino_shapes_150.pdf(", "pdf");
