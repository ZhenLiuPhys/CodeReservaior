...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   Int_t n_passaa1;//Added for the study of Tao Han, David Krohn, Lian-Tao Wang, Wenhan Zhu's paper
   Int_t n_passaa2;
   Int_t n_passjj1;
   Int_t n_passjj2;
   Int_t n_passjj3;
...
//   mytree->Branch("vl_pt",&vl_pt,"vl_pt/D");
   ...
   mytree->Branch("n_passaa1",&n_passaa1, "n_passaa1/I");
   mytree->Branch("n_passaa2",&n_passaa2, "n_passaa2/I");
   mytree->Branch("n_passjj1",&n_passjj1, "n_passjj1/I");
   mytree->Branch("n_passjj2",&n_passjj2, "n_passjj2/I");
   mytree->Branch("n_passjj3",&n_passjj3, "n_passjj3/I");
   mytree->Branch("n_passjj4",&n_passjj4, "n_passjj4/I");
   mytree->Branch("n_passaajj1",&n_passaajj1, "n_passaajj1/I");
   mytree->Branch("n_passaajj2",&n_passaajj2, "n_passaajj2/I");
   mytree->Branch("n_passall", &n_passall, "n_passall/I");
...
*/
	...
	if(Particle_Status[i]==1){
	  if(Particle_PID[i]==21||TMath::Abs(Particle_PID[i])<=5){
	     ...
	     photon_temp.SetPxPyPzE(Particle_Px[i],Particle_Py[i],Particle_Pz[i],Particle_E[i]);
             photon_all.SetPxPyPzE(photon_all.Px()+Particle_Px[i],photon_all.Py()+Particle_Py[i],photon_all.Pz()+Particle_Pz[i],photon_all.E()+Particle_E[i]);
      ...
      TLorentzVector a1, a2, jet1, jet2;
	...
	if(a1.Pt()>55*photon_all.Mag()/120) n_passaa1=1;
        if(a2.Pt()>25) n_passaa2=1;
      }
	...
	temp2 = jet.at(n-1);
        ...
        if(jet1.Pt()>30&&TMath::Abs(jet1.Eta())<4.7) n_passjj1=1;
        if(jet2.Pt()>20&&TMath::Abs(jet2.Eta())<4.7) n_passjj2=1;
        if(TMath::Abs(jet1.Eta()-jet2.Eta())>3.5) n_passjj3=1;
        if(jet_all.Mag()>350) n_passjj4=1;
       }

      if(n_jets==2&&n_a==2){
        if(TMath::Abs(a1.Eta()+a2.Eta()-jet1.Eta()-jet2.Eta())<5) n_passaajj1=1;
        if(deltaphi(jet_all.Phi(), photon_all.Phi())>2.6) n_passaajj2=1;
      }
      ...
      if(n_jets == 2){
        ...
        temp1 = jet.at(n-2);
        temp2 = jet.at(n-1);
        if(temp1.E()>500 && TMath::Abs(temp1.Eta())>3 && TMath::Abs(temp1.Eta()) <5 && temp1.Pt()>40){
          n_passjj = 1;
          temptag = temp1;
          tempveto = temp2;
        }
        else if(temp2.E()>500 && TMath::Abs(temp2.Eta())>3 && TMath::Abs(temp2.Eta()) <5 && temp2.Pt()>40){
            n_passjj = 1;
            temptag = temp2;
            tempveto = temp1;
	}
      ...
      if(n_passaa1==1&&n_passaa2==1&&n_passjj1==1&&n_passjj2==1&&n_passjj3==1&&n_passjj4==1&&n_passaajj1==1&&n_passaajj2==1) n_passall==1;
