...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   mytree->Branch("m_trr_clustered",&m_trr_clustered,"m_trr_clustered/D");
...
for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(Particle_Status[i]==1){
	  if(Particle_PID[i]==21||abs(Particle_PID[i])<=5){
	     if(abs(Particle_PID[i])==5) n_b++;
	     n_jets++;
	   ...
	   else if(abs(Particle_PID[i])==11||abs(Particle_PID[i])==13||abs(Particle_PID[i])==15){
	     n_leptons++;
	   ...
	   else if(abs(Particle_PID[i])==12||abs(Particle_PID[i])==14||abs(Particle_PID[i])==16){
	     n_vl++;
	     vl_pt=Particle_PT[i];
      ...
      m_tr=(met+lept).Mag();
      m_trr=(lepton_all+vlt).Mag();
      lept.SetE(sqrt(pow(lept.Pt(),2) + lepton_all.Mag2()));
      m_tr_clustered=(lept+met).Mag();
      m_trr_clustered=(lept+vlt).Mag();
	...
	if(n_jets==4&&n_leptons==3){
	  ...
	  inv_23 = temp_23.Mag();
	    ...
	    if(abs(leptypes.at(nlep-3))==abs(leptypes.at(nlep-2))&&abs(leptypes.at(nlep-3))==abs(leptypes.at(nlep-1))){
	    ...
	    else if(abs(leptypes.at(nlep-3))==abs(leptypes.at(nlep-2))){
	      z1 = temp3;
	      wlep = lepton.at(nlep-1);
	    }
	    else if(abs(leptypes.at(nlep-3))==abs(leptypes.at(nlep-1))){
	      z1 = temp2;
	      wlep = lepton.at(nlep-2);
	    }
	    else if(abs(leptypes.at(nlep-2))==abs(leptypes.at(nlep-1))){
	      z1 = temp1;
	      wlep = lepton.at(nlep-3);
	    }
	    else lepflag = 1;
