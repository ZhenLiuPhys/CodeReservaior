...
void higgsino::Begin(TTree * /*tree*/)
{
   ...
   // t1->Branch("MT_1", &MT_1, "MT_1/F");
   // t1->Branch("MT_2", &MT_2, "MT_2/F");
   //t1->Branch("MLL", &MLL, "MLL/F");
   ...
   // t1->Branch("DRLL", &DRLL, "DRLL/F");
   //t1->Branch("DPHILL", &DPHILL, "DPHILL/F");
   //t1->Branch("TAMULT", &TAMULT, "TAMULT/I");
   //t1->Branch("TAPT", TAPT, "TAPT[TAMULT]/F");
...
Bool_t higgsino::Process(Long64_t entry)
{
  ...
  //  if (bjets.size() > 1 && taus.size() == 1 && leptons.size() == 1){
  if (jets.size() >= 0 && bjets.size() > 0 && leptons.size() == 1 && taus.size() == 1){
    ...
    // TLorentzVector vis = leptons[0] + taus[0] + bjets[0] + bjets[1];
